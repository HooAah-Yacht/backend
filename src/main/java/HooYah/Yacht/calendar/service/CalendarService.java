...
public class CalendarService {
    ...
    public CalendarInfo createCalendar(CalendarCreateRequest request, User user) {
        ...
        if (request.getType() == CalendarType.PART && part != null) {
            ...
            if (!existingPartCalendars.isEmpty()) {
                Calendar existingAutoCalendar = existingPartCalendars.getLast(); // get last
                ...
                if (completed && user != null) {
                    ...
                    addRepair(part, user, existingAutoCalendar.getReview(), request.getStartDate());
    ...
    public CalendarInfo updateCalendar(Long id, CalendarUpdateRequest request, User user) {
        ...
        if (isCompleting && user != null && request.getType().equals(CalendarType.PART)) {
            addRepair(part, user, calendar.getReview(), calendar.getStartDate());
            calendarAlarmAutoGeneratorService.generate(part);
        }
    ...
    public void deleteCalendar(Long id, User user) {
        ...
        Part part = calendar.getPart();
        CalendarType type = calendar.getType();
        
        calendarRepository.delete(calendar);

        if(type.equals(CalendarType.PART))
            calendarAlarmAutoGeneratorService.generate(part);
    }
...
//    }
    ...
    private void addRepair(Part part, User user, String content, OffsetDateTime repairDate) {
        repairService.addRepair(part.getId(), content, repairDate, user);
    }
